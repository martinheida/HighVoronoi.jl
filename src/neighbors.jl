struct NeighborFinder{S,T}
    dimension::Int64
    candidates::Vector{Int64}
    broken::Vector{Bool}
    verteces::Vector{T}
    transformed::MVector{S,Float64}
    local_basis::Vector{MVector{S,Float64}}
    local_origin::MVector{S,Float64}
    cell_center::MVector{S,Float64}
...
function NeighborFinder(dim,x)
    return NeighborFinder{dim,typeof(x)}(dim,[1],[false],Vector{typeof(x)}(undef,1),MVector{dim}(zeros(Float64,dim)),empty_local_Base(dim),MVector{dim}(zeros(Float64,dim)),MVector{dim}(zeros(Float64,dim)),Int64[1],[[true]],[false])
end
...
struct IterativeDimensionChecker{S}
    ...
    edge_iterator::FastEdgeIterator{DimFEIData{S}}
