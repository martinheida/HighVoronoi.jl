...
struct LLoyd{II,F,MC}
    ...
    function LLoyd(mi,tol=Inf64; integrator=VI_POLYGON, tolerance_function=nothing, mc_accurate=(1000,5,20),global_tolerance=true,local_tolerance=false,use_voronoi_data=false, local_tol=Inf64, global_tol=Inf64)
        ...
        tol_lo = min(tol,local_tol)
        return new{typeof(integrator),typeof(tolerance_function2),typeof(mc_accurate)}(mi,tol,integrator,tolerance_function2,mc_accurate,global_tolerance,local_tolerance,use_voronoi_data, tol_lo, min(tol,global_tol))    
    end
end
