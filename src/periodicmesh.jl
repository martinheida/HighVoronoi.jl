...
function adjust_periodic_boundaries(I,periodic,periodicity;integrator,integrand,mc_accurate)
    ...
    I2 = Integrator(new_mesh,integrator,integrand=integrand,mc_accurate=mc_accurate)
...
function PeriodicVoronoiGeometry(xs::Points;kwargs...)
    dispatch_resolve = VoronoiNode(xs[1])
    #@descend PeriodicVoronoiGeometry(_Matrix_from_Points(xs),dispatch_resolve;kwargs...)
    #error("")
    return PeriodicVoronoiGeometry(_Matrix_from_Points(xs),dispatch_resolve;kwargs...)
end
...
function PeriodicVoronoiGeometry(matrix_data::Matrix,dispatch_resolve; silence=false, search_settings=NamedTuple(), fast=true, periodic=[], scale=ones(Float64,size(matrix_data,1)), repeat = 2*ones(Int64,size(matrix_data,1)), dimensions=ones(Float64,size(matrix_data,1)), integrator=VI_POLYGON, integrand=nothing, mc_accurate=(1000,100,20))
    dim = size(matrix_data,1)
    cancel = false
    try
        check_boundary(VoronoiNodes(matrix_data,length(dispatch_resolve)),cuboid(dim,periodic=[],dimensions=0.999*dimensions,offset=0.0005*dimensions))
    catch 
        cancel = true
    end
    cancel && error(("The nodes $(VoronoiNodes(matrix_data,length(dispatch_resolve))) have to lie inside the following domain with 0.5% distance to the boundary: \n"*boundaryToString(cuboid(dim,periodic=[],dimensions=dimensions),offset=4)))
    ...
    if number_of_nodes==1
        return CubicVoronoiGeometry(matrix_data,dispatch_resolve, search_settings=search_settings, fast=fast, periodic=periodic, scale=scale, repeat = repeat, dimensions=dimensions, integrator=integrator, integrand=integrand, mc_accurate=mc_accurate)
    end
    ...
    if fast
        ...
        Integrator = HighVoronoi.Integrator(Voronoi_MESH(xs),my_integrator,integrand=integrand,mc_accurate=mc_accurate)
...
#                vp_line_up()
        ...
        #_domain = Discrete_Domain(cube,shifts,reference_shifts, reference,extended_cube)
        I2=HighVoronoi.Integrator(I.Integral.MESH,integrator,integrand=integrand,mc_accurate=mc_accurate)
    ...
    for i in old_range
        ...
        for (sig,r) in mesh.All_Verteces[i]
            ...
            if !(trust_all)
                ...
                new_node, dist =_nn(searcher.tree,r2,x->(x in sig2))
                measure = 0.0
                for s in sig2
