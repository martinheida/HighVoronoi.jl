...
function VoronoiStatistics(dim,samples;periodic=nothing,points=1,my_generator=nothing,geodata=true)
    ...
    for S in 1:samples
        ...
        I,_=voronoi(xs,searcher=Raycast(xs,allow_irregular=(periodic!=nothing),force_irregular_search=(periodic!=nothing)),intro="Run number: $S  ",compact=true, Iter=1:number)
        vp_line_up()
        I2=Integrator(I.Integral.MESH,type=geodata ? VI_POLYGON : VI_GEOMETRY,integrand=nothing)
