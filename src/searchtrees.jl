...
mutable struct NNSearchData{T,T2,T3}
    ...
    horizontal_error::Float64 # x_0 ⋅ u
    ...
    function NNSearchData(p::P,nodes::Int,lmesh) where P
        ...
        new_mode = false
        new{P,typeof(point),typeof(hp)}(IncrementalInt64Vector(len_p), 0.0, MVector{1, Float64}([0.0]), MVector{1, Int64}([0]),IncrementalInt64Vector(length(p)+1),Bool[],0.0,0.0,0.0,0*p,0,0,0.0,0.0,0.0, 0*p,0*p,0*p,0*p,MVector(0*p),MVector(0*p),lmesh,0.0,falses(2),0.0,hp,Vector{P}(undef,size(P)[1]),0.0,0.0,0.0,false,IncrementalFloat64Vector(len_p),IncrementalFloat64Vector(len_p),IncrementalFloat64Vector(len_p),new_mode,IncrementalInt64Vector(len_p),false,0*p,0*p)
    end
end
function reset!(nn_data::NNSD,taboo,r,x0,u,plane_tolerance,xs,edge=Int64[],du=0.0, cast_type=Raycast_By_Walkray(),full_edge=Int64[]) where {T,T2,T3,NNSD<:NNSearchData{T,T2,T3}}
    reset!(nn_data.sigma)
    ...
    nn_data.horizontal_error = abs(maxmeas-minmeas)
    #println(nn_data.horizontal_error)
...
function skip_nodes_on_search(data::NNSearchData{T},x_new,i,dist,boundarymode::S) where {T,S<:StaticBool}
    ...
    plane_tolerance = abs(full_error/new_t) # relative error
    this_error = full_error + data.horizontal_error

    _dnrx02 = data.dist_new_r_x0_2
    correction = max(_dnrx02 * 10 * plane_tolerance,10*this_error)
    this_correction = max(correction,data.full_error_t)
    i==0 && println("hier2: $_dnrx02 vs. $new_dist vs. $correction")
    #println("--------------------------------------------------------------------------")
    #println("corr: $correction,   this_corr: $(this_correction)  dfet: $(data.full_error_t)")
    if new_dist > _dnrx02 + this_correction  # by no means a better candidate
        return true
    end
...
#    x_new = xs[i]
    ...
    abs_δx = dot(δx,δx)
        ...
        #if 148 in data.sigma
        #println("$i: $(abs(new_dist - _dnrx02)), $correction")
    #end

    scale = in_bounds(data.box_mins,data.box_maxs,r0) ? 1.0 : sqrt(data.dist_r_x0_2 / (new_t^2))

    if abs(new_dist - _dnrx02) < max(correction,data.full_error_t) # as good as current candidate => degenerate vertex?
        abs_δx/new_dist<100*correction && (return true)
        push!(data.sigma,i)
        push!(data.cs, c_new)
        push!(data.ts, new_t)
        push!(data.full_errors,full_error)    
        data.full_error_t = max( min(1,10*scale)*abs(new_dist - _dnrx02), data.full_error_t )
        #println("ende hier")
    ...
    #error("scale issue here to be solved")
    ...
    data.full_error_t = min(1,10*scale)*max(abs(dnrx02-sum(abs2,x_new-new_r)),this_error) # (abs(data.t)+full_error)^2+data.dist_r_x0_2-dnrx02
    #data.full_error_t = (abs(data.t)+10*dim*full_error)^2+data.dist_r_x0_2-dnrx02
    

    #newtry = data.bestdist[1]==typemax(Int64)
    
    data.bestdist[1] = dnrx02 + min(1,10*scale)*data.full_error_t
        #data.full_error_t>1.0E-12 && error("$full_error, $new_t, $dnrx02, $(data.dist_r_x0_2)")
...
struct UnstructuredTree{P <: Point,T,NNSD<:NNSearchData{P}} <: AbstractTree{P}  # Making UnstructuredTree a subtype of HVTree
    ...
    set_bounding_box(tree::UnstructuredTree,mins,maxs) = begin
        tree.data.box_mins = mins 
        tree.data.box_maxs = maxs
    end
