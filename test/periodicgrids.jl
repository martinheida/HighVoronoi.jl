@testset "Periodic Grids" begin
    function test_periodic_mesh_integration(dim,nn,f=true,peri=[],integrator=HighVoronoi.VI_POLYGON)
        ...
        VG = VoronoiGeometry( VoronoiNodes(rand(dim,nn)),periodic_grid=(periodic=peri, dimensions=ones(Float64,dim), 
                                scale=0.25*ones(Float64,dim), repeat=4*ones(Int64,dim),fast=f), search_settings=(method=RCCombined,),integrator=integrator,integrand=x->[1.0,x[1]^2,x[2]^2],silence=global_silence)
    ...
    @test test_periodic_mesh_integration(5,2,false)
